...
DEDUPE_CACHE_FILE = Path(__file__).resolve().parent.parent.parent / "config" / "data" / "correlation_dedupe_cache.json"
DEDUPE_PROGRESS_DB_FILE = Path(__file__).resolve().parent.parent.parent / "config" / "data" / "correlation_dedupe_progress.db"
...
class GalleryService:
    ...
    def _enrich_image_info(
        ...
        try:
            ...
            if include_dimensions and ('width' not in image_info or 'height' not in image_info):
                try:
            ...
            if include_description and 'description' not in image_info:
                try:
                    description = self._get_image_description_from_folder(image_path.parent, image_path.name)
                    image_info['description'] = description
                    image_info['has_description'] = description is not None and description.strip() != ""
                except Exception as e:
    ...
    def _get_image_description_from_folder(self, folder_path: Path, filename: str) -> Optional[str]:
        """按现有优先级读取单张图片描述"""
    ...
    def _build_dedupe_control_state(
        ...
        if kept_factors >= target_limit and target_limit < DEDUPE_MAX_KEPT_FACTORS:
            next_target_kept = min(target_limit + DEDUPE_BATCH_SIZE, DEDUPE_MAX_KEPT_FACTORS)
        ...
        return {
            ...
            'has_partial_result': kept_factors > 0 and processed_factors < total_factors,
        }

    def _build_dedupe_run_key(self, cache_key: str, signature: str, threshold: float) -> str:
    ...
    def _ensure_dedupe_run(
        ...
        conn.execute(
            ...
            """,
            (run_key, cache_key, signature, threshold, total_factors, 'running', now, now),
        )
        conn.commit()
    ...
    def _get_factor_data_file_path(self, factor_version: str, factor_name: str) -> Path:
        ...
        parquet_factor_name = factor_name[:-5] if factor_name.endswith('_fold') else factor_name
    ...
    def _build_dedupe_cache_signature(self, images: List[Dict], threshold: float) -> str:
        ...
        for image_info in images:
            ...
            if parquet_key not in parquet_mtime_cache:
                parquet_path = self._get_factor_data_file_path(factor_version, factor_name)
    ...
    def _get_cached_dedupe_entry(self, cache_key: str, signature: str) -> Optional[Dict]:
        """读取匹配签名的本地去重缓存条目"""
    ...
    def _get_cached_dedupe_result(self, cache_key: str, signature: str) -> Optional[List[str]]:
        """读取命中的本地去重结果"""
    ...
    def _load_existing_dedupe_snapshot(
        ...
        processed_prefix, kept_images = self._load_resumable_dedupe_state(conn, run_key, images)
    ...
    def _load_dedupe_annotations(
        ...
        for row in rows:
            ...
            target_factor_name = (
                row['max_corr_factor_name']
                or (self._get_factor_name_from_image(target_image) if target_image else None)
            )
            target_factor_version = (
                row['max_corr_factor_version']
                or (str(target_image.get('factor_version')) if target_image else None)
            )

            annotations[relative_path] = {
                'dedupe_compared_count': int(row['compared_count'] or 0),
                'dedupe_max_corr': float(row['max_corr']) if row['max_corr'] is not None else None,
                'dedupe_max_corr_with_relative_path': str(target_path) if target_path else None,
    ...
    def _apply_neu_ret_metadata(
        ...
        for image_info in images:
            ...
            image_info['factor_version'] = image_info.get('factor_version', default_factor_version)
    ...
    def _calculate_mean_factor_correlation(
        ...
        image_b: Dict,
        factor_cache: Dict[Tuple[str, str], Optional[object]],
        correlation_cache: Dict[Tuple[Tuple[str, str], Tuple[str, str]], Optional[float]],
    ) -> Optional[float]:
        """计算两个因子的平均截面相关性"""
        factor_a = (
        ...
        try:
            ...
            if len(common_dates) == len(index1) and len(common_dates) == len(index2) and index1.equals(index2):
                values1 = df1['values']
                values2 = df2['values']
            else:
            ...
            if mean_corr is None or (isinstance(mean_corr, float) and math.isnan(mean_corr)) or mean_corr != mean_corr:
                correlation_cache[cache_key] = None
                return None
    ...
    def _emit_dedupe_progress(
        ...
        try:
            ...
            if total_factors > 0:
                progress_percent = min(100, int((processed_factors / total_factors) * 100))

            if processed_factors > 0 and total_factors > processed_factors and elapsed_seconds > 0:
                eta_seconds = (elapsed_seconds / processed_factors) * (total_factors - processed_factors)
    ...
    def _dedupe_images_by_correlation(
        ...
        try:
            ...
            run_key = self._build_dedupe_run_key(dedupe_cache_key, cache_signature, threshold)
            ...
            if cache_key:
                cached_paths = self._get_cached_dedupe_result(cache_key, cache_signature)
                if cached_paths is not None:
                    cached_images = self._apply_cached_dedupe_paths(images, cached_paths)
                    cached_annotations = self._load_dedupe_annotations(
                        images,
                        cache_key=cache_key,
                        threshold=threshold,
                    )
                    cached_images = self._apply_dedupe_annotations(cached_images, cached_annotations)
                    streamed_images = [
                    ...
                    final_state = self._build_dedupe_control_state(
                        ...
                        target_kept_limit=max(normalized_target_kept, len(cached_images)),
                    )
                    self._emit_dedupe_progress(
                        task_id,
                        status='completed',
            ...
            if existing_snapshot is not None and not continue_requested:
                ...
                if cache_key and kept_images:
                    self._save_cached_dedupe_result(
                        cache_key,
                        cache_signature,
                        [str(image.get('relative_path')) for image in kept_images if image.get('relative_path')],
                        status=run_status,
            ...
            all_image_map = {str(image.get('relative_path', '')): image for image in images}
            ...
            if processed_prefix > 0:
                kept_images = self._apply_dedupe_annotations(kept_images, existing_annotations)
                restored_images = [
                    self._enrich_image_info(dict(image), include_dimensions=False)
                    for image in kept_images
                ]
            ...
            if len(kept_images) >= normalized_target_kept and normalized_target_kept < DEDUPE_MAX_KEPT_FACTORS:
                final_state = self._build_dedupe_control_state(
                    status='paused',
                    processed_factors=processed_prefix,
                    total_factors=total_factors,
            ...
            for index, image_info in enumerate(images, start=1):
                ...
                saved_comparisons = self._load_saved_factor_comparisons(conn, run_key, index)
                should_hide = False
                ...
                if max_corr_with:
                    ...
                    if target_image is not None:
                        image_info['dedupe_max_corr_factor_name'] = self._get_factor_name_from_image(target_image)
                        image_info['dedupe_max_corr_factor_version'] = str(target_image.get('factor_version', ''))
            ...
            logger.exception("高相关去重中断 %s: %s", cache_key or task_id or 'unknown', e)
    ...
    def get_image_list(
        ...
        try:
            ...
            result['images'] = [self._enrich_image_info(image) for image in result['images']]
            if sort_by == 'neu_ret' and dedupe_source_images is not None:
                annotations = self._load_dedupe_annotations(
                    dedupe_source_images,
                    cache_key=f"single::{folder_name}",
                )
                result['images'] = self._apply_dedupe_annotations(result['images'], annotations)
            if dedupe_state:
    ...
    def search_images_in_selected_subfolders(self, parent_folder: str, query: str, selected_subfolders: List[str], page: int = 1, per_page: int = 20) -> Dict:
        ...
        try:
            ...
            for subfolder_name in selected_subfolders:
                ...
                if not subfolder.exists() or not subfolder.is_dir() or subfolder.name.startswith('.'):
                    continue
